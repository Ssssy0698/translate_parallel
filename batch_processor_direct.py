...
# 导入公共函数
try:
    from final_translate import (
        load_tibetan_file, 
        create_json_template
    )
    from improved_translator import ImprovedTibetanTranslator
    HAS_IMPORTS = True
...
class BatchTranslatorDirect:
    ...
    def __init__(self, max_workers=20, output_dir="results"):  # 默认改为20
        ...
        # 资源监控
        print(f"初始化批处理器 - 最大并行数: {self.max_workers}")
    ...
    def translate_all_lines(self, lines, file_type, output_file, file_path):
        ...
        try:
            # 每个线程创建自己的翻译器实例
            with ImprovedTibetanTranslator(headless=True) as translator:
...
def main():
    parser = argparse.ArgumentParser(description='藏语批量翻译处理器 (直接调用版本)')
    ...
    if args.workers > 10:
        ...
        print("建议:")
        print("1. 确保有足够的内存 (至少 {args.workers*0.5:.1f}GB)")
